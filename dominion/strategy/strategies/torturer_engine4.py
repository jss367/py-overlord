...
class TorturerEngine(EnhancedStrategy):
    ...
    def __init__(self) -> None:
        ...
        def need_more_villages(_s, me):
            terminals = me.count_in_deck("Torturer") + me.count_in_deck("Patrol") + me.count_in_deck("Taskmaster")
        ...
        # Gain priorities
        self.gain_priority = [
            # Provinces at 8+, but only when engine is online or game is close
            PriorityRule(
                "Province",
                PriorityRule.and_(
                    PriorityRule.resources("coins", ">=", 8),
                    PriorityRule.or_(
                        lambda s, me: me.count_in_deck("Torturer") >= 3,
                        lambda s, me: s.supply.get("Curse", 0) == 0,
                        lambda s, me: s.supply.get("Patrician", 99) <= 2 or s.supply.get("Snowy Village", 99) <= 2,
                    ),
                ),
            ),
            ...
            # Earlier first Inn to stabilize after first Torturer
            PriorityRule(
                "Inn",
                PriorityRule.and_(
                    PriorityRule.resources("coins", ">=", 5),
            ...
            # Allow a second Troupe mid-game when Villagers are low and terminals are many
            PriorityRule(
                "Acting Troupe",
                PriorityRule.and_(
                    PriorityRule.resources("coins", ">=", 3),
                    lambda _s, me: me.count_in_deck("Acting Troupe") < 2,
                    lambda _s, me: me.count_in_deck("Torturer") + me.count_in_deck("Patrol") >= 3,
                    lambda _s, me: getattr(me, "villagers", 0) <= 1,
                ),
            ),
            ...
            # Silver fallback if we're whiffing $5s and engine parts present
            PriorityRule(
                "Silver",
                PriorityRule.and_(
                    PriorityRule.resources("coins", ">=", 3),
                    ...
                    lambda _s, me: me.count_in_deck("Torturer") >= 1,
                    lambda _s, me: me.count_in_deck("Gold") == 0,
                    lambda _s, me: me.count_in_deck("Patrol") >= 1,
                ),
            ),
            ...
            PriorityRule(
                ...
                PriorityRule.and_(
                    PriorityRule.resources("coins", ">=", 5),
                    # Prefer actions-in-play if available; otherwise use a safer plays threshold
                    lambda s, me: getattr(s, "actions_in_play", lambda _me: 0)(me) >= 5 or me.actions_this_turn >= 6,
                    # Do not pre-empt core engine pieces
                    lambda _s, me: me.count_in_deck("Torturer") >= 3,
            ...
            # Greening & pile control (moved above Patrician-at-2)
            PriorityRule(
                "Duchy",
                PriorityRule.or_(
                ...
                PriorityRule.or_(
                    ...
                    PriorityRule.and_(
                        lambda s, _me: (
                            ...
                            + (1 if s.supply.get("Emporium", 99) <= 1 else 0)
                        )
                        >= 2,
                    ),
                ),
            ),
            # Patrician at 2 coins (after pile-out checks)
