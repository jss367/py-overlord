...
class GameState:
    ...
    def _resolve_prepared_cards(self, player: PlayerState) -> None:
        ...
        for card in cards:
            # Only Action and Treasure cards can be played; non-playable
            # cards (pure Victory, Curse) are discarded directly.
            if not (card.is_action or card.is_treasure):
                self.discard_card(player, card, from_cleanup=False)
                continue
